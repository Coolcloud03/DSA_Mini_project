...
public class OSMGraphBuilder {
    ...
    public Graph buildGraph(File osmFile) {
                    ...
                    switch (qName) {
                        ...
                        case "nd": {
                            if (inWay) {
                                ...
                                if (ref != null)
                                    wayNodeRefs.add(Long.parseLong(ref));
                            }
                            break;
                        }
                        case "tag": {
                            if (inWay) {
                                ...
                                if ("oneway".equals(k)) {
                                    if ("-1".equals(v))
                                        wayOnewayReverse = true;
                                    if ("yes".equalsIgnoreCase(v) || "true".equalsIgnoreCase(v) || "1".equals(v))
                                        wayOnewayForward = true;
                                }
                            }
                            break;
                        }
                ...
                public void endElement(String uri, String localName, String qName) {
                    if ("way".equals(qName)) {
                        if (wayIsHighway && wayNodeRefs.size() >= 2) {
                            usedNodeIds.addAll(wayNodeRefs);
        ...
        // Filter nodes to only keep those used in highways
        nodes.keySet().retainAll(usedNodeIds);
